    ...
    <div class="card p-4">
      <form method="POST" action="{{ route('admin.devices.update',$device) }}">
        ...
        <div class="mb-3">
          ...
          <input type="text" name="serial_number" value="{{ $device->serial_number }}" class="form-control @error('serial_number') is-invalid @enderror">
        ...
        <div class="mb-3">
          ...
          <select name="category_id" class="form-select @error('category_id') is-invalid @enderror">
            ...
            @foreach($categories as $c)
              <option value="{{ $c->id }}" {{ $device->category_id === $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
            @endforeach
          </select>
        ...
        <div class="mb-3">
          ...
          <textarea name="condition" class="form-control @error('condition') is-invalid @enderror" rows="3">{{ $device->condition }}</textarea>
